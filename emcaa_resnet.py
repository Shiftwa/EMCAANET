...
class emcaa_layer(nn.Module):

    def __init__(self, channel, k_size=3):
        super(emcaa_layer, self).__init__()
...
class EMCAABottleneck(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, downsample=None, k_size=3):
        super(EMCAABottleneck, self).__init__()
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.skeca = emcaa_layer(planes, k_size)
...
def emcaanet_resnet50(k_size=[3, 3, 7, 7], num_classes=1000):

    model = EMCAAResNet(EMCAABottleneck, [3, 4, 6, 3], num_classes=num_classes, k_size=k_size)
    return model
...
if __name__ == "__main__":
    for net_name in __all__:
        if net_name.startswith('emcaanet_'):
            print(net_name)
